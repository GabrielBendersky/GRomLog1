...
Romer_story_diaria = int(
    input("Digite 1 para iniciar a compra ou 2 para finalizar o expediente: ")
)
...
while Romer_story_diaria != 2:
    if Romer_story_diaria == 1:
        ...
        if valor_compra > 200:
            ...
            print(
                f"Desconto de 10% aplicado! Valor final: R$ {valor_compra:.2f}"
            )
        elif valor_compra > 100:
            desconto = valor_compra * 0.05
            valor_compra -= desconto
            print(
                f"Desconto de 5% aplicado! Valor final: R$ {valor_compra:.2f}"
            )
        else:
        ...
        # PARCELAMENTO
        Parcelar = int(
            input("Deseja parcelar a compra? Digite 1 para sim ou 0 para não: ")
        )
        if Parcelar == 1:
            quantidade_de_parcelas = int(
                input("Digite a quantidade de parcelas desejadas (1 a 6): ")
            )
            if 1 <= quantidade_de_parcelas <= 6:
                valor_parcela = valor_compra / quantidade_de_parcelas
                print(
                    f"Compra parcelada em {quantidade_de_parcelas}x de R$"
                    f" {valor_parcela:.2f}"
                )
            else:
                print(
                    "Quantidade de parcelas inválida. A compra será à vista."
                )
        else:
...
print(
    "Obrigado por utilizar o auto atendimento ROMER'S STOR&. Tenha um ótimo"
    " dia!E NÃO COMPRE NA LOJA DO YURI"
)
